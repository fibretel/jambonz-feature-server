 ...
 * active-fs: srf.locals.serviceUrl does not exist until the express server has
 * actually bound (lib/utils/http-listener.js sets it in the listen callback).
 * That listener also walks the port forward on EADDRINUSE up to HTTP_PORT_MAX,
 * so the URL is not knowable ahead of the bind and CAN legitimately differ
 * between restarts -- which is exactly why the changed-address handling below
 ...
 * it there.
 *
 * @param {Srf} srf - the srf instance; srf.locals.serviceUrl must be set, i.e.
 *   call this only after the HTTP listener has bound
 * @param {object} logger
 * @param {object} opts
