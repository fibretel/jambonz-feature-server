...
const {registerActiveFs, unregisterActiveFs} = require('./active-fs-registration');
const {registerFsServiceUrl, unregisterFsServiceUrl} = require('./fs-service-url-registration');
...
module.exports = (logger) => {
  ...
  if (AWS_SNS_TOPIC_ARN && AWS_REGION) {
      ...
      try {
        ...
        lifecycleEmitter
          ...
          .on(LifeCycleEvents.ScaleIn, async() => {
            ...
            const {srf} = require('../..');
            /* the OPTIONS ping below tells the SBCs we are closed, but sbc-inbound
               picks a feature server straight out of `<cluster>:active-fs`; leave it */
            await unregisterActiveFs(srf, logger, {clusterId: JAMBONES_CLUSTER_ID});
            /* REST-originated calls are new calls too: api-server picks a feature
               server out of `<cluster>:fs-service-url` and never sees the OPTIONS
               ping at all, so leaving active-fs alone does not dry us up. */
          ...
          .on(LifeCycleEvents.StandbyEnter, async() => {
            dryUpCalls = true;
            const {srf} = require('../..');
            await unregisterActiveFs(srf, logger, {clusterId: JAMBONES_CLUSTER_ID});
            await unregisterFsServiceUrl(srf, logger, {clusterId: JAMBONES_CLUSTER_ID});
          ...
          .on(LifeCycleEvents.StandbyExit, () => {
            ...
            /* symmetric with StandbyEnter: coming back means taking REST-originated
               calls again, and the http listener is not re-created here so nothing
               else would re-advertise us to api-server */
            registerFsServiceUrl(srf, logger, {clusterId: JAMBONES_CLUSTER_ID});
            pingProxies(srf);
    ...
    process.on('SIGUSR1', async() => {
      ...
      const {srf} = require('../..');
      /* drying up means no NEW calls; sbc-inbound routes new calls from
         `<cluster>:active-fs`, so leaving that set is what actually dries us up */
      await unregisterActiveFs(srf, logger, {clusterId: JAMBONES_CLUSTER_ID});
      /* ...and api-server routes REST-originated calls and messaging from
         `<cluster>:fs-service-url`, which is a second, independent source of new
         work. Leaving only active-fs drains the SIP half and keeps taking REST
         dials for as long as the process lives, because the fs-service-url
         refresh timer keeps re-asserting membership every 30s. */
      await unregisterFsServiceUrl(srf, logger, {clusterId: JAMBONES_CLUSTER_ID});
      const {writeSystemAlerts} = srf.locals;
      if (writeSystemAlerts) {
        const {SystemState, FEATURE_SERVER} = require('./constants');
